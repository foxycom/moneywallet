...
JAVA_OPTS=" -Dabc.instrument.array.operations -Dabc.instrument.fields.operations -Dabc.taint.android.intents -Dabc.instrument.include=com.oriondev.moneywallet"
...
if [ "$$IS_RUNNING" == "0" ]; then \
        ...
        APP_INSTALLED=$$($(ADB) shell pm list packages | grep -c "com.oriondev.moneywallet.dev$$"); \
        INSTRUMENTATION_TESTS_INSTALLED=$$($(ADB) shell pm list packages | grep -c "com.oriondev.moneywallet.dev.test$$"); \
        if [ "$$APP_INSTALLED" == "1" ]; then \
				(>&2 echo "Removing app"); \
                $(ADB) uninstall com.oriondev.moneywallet.dev > /dev/null; \
        else \
                (>&2 echo "App not installed"); \
        fi; \
        if [ "$$INSTRUMENTATION_TESTS_INSTALLED" == "1" ]; then \
				(>&2 echo "Removing test app"); \
                $(ADB) uninstall com.oriondev.moneywallet.dev.test > /dev/null; \
        else \
...
$(shell if [ "$(TRACING_APKS_INSTALLED)" == "0" ]; then \
	...
	$(ADB) shell pm clear com.oriondev.moneywallet.dev > /dev/null; \
fi)
...
app-original.apk : 
	...
	$(GW) -PjacocoEnabled=false assembleFlossOsm && \
	mv app/build/outputs/apk/flossOsm/debug/app-floss-osm-debug.apk app-debug.apk && \
...
app-androidTest.apk :
	...
	mv app/build/outputs/apk/androidTest/flossOsm/debug/app-floss-osm-debug-androidTest.apk app-androidTest-unsigned.apk && \
...
app-original-for-coverage.apk app-androidTest-for-coverage.apk:
	@export ABC_CONFIG=$(ABC_CFG) && \
	$(GW) -PjacocoEnabled=true clean assembleFlossOsm assembleAndroidTest && \
	mv app/build/outputs/apk/flossOsm/debug/app-floss-osm-debug.apk app-original-for-coverage.apk  && \
	mv app/build/outputs/apk/androidTest/flossOsm/debug/app-floss-osm-debug-androidTest.apk app-androidTest-for-coverage.apk
...
#	Log directly to the expected file
	$(ADB) shell am instrument -w -e class $(TEST_NAME) com.oriondev.moneywallet.dev.test/androidx.test.runner.AndroidJUnitRunner 2>&1 | tee $(@)
#	Copy the traces if the previous command succeded
	@export ABC_CONFIG=$(ABC_CFG) && $(ABC) copy-traces com.oriondev.moneywallet.dev ./traces/$(TEST_NAME) force-clean
