...
JAVA_OPTS=" -Dabc.instrument.fields.operations -Dabc.taint.android.intents -Dabc.instrument.include=com.oriondev.moneywallet"
...
app-original.apk :
	...
	mv app/build/outputs/apk/flossOsm/debug/app-floss-osm-debug.apk . && \
	$(ABC) sign-apk app-floss-osm-debug.apk && \
	mv -v app-floss-osm-debug.apk app-original.apk

app-original-with-jacoco.apk :
	export ABC_CONFIG=$(ABC_CFG) && \
	$(GW) -PjacocoEnabled=true assembleFlossOsm && \
	mv app/build/outputs/apk/flossOsm/debug/app-floss-osm-debug.apk . && \
	$(ABC) sign-apk app-floss-osm-debug.apk && \
	mv -v app-floss-osm-debug.apk app-original.apk
...
espresso-tests.log : app-original.apk app-androidTest.apk running-emulator
	...
	$(ADB) shell am instrument -w -r com.oriondev.moneywallet.dev.test/androidx.test.runner.AndroidJUnitRunner | tee espresso-tests.log
	export ABC_CONFIG=$(ABC_CFG) && $(ABC) stop-all-emulators
	$(RM) running-emulator
...
%.testlog: app-androidTest.apk app-instrumented.apk running-emulator
	...
	echo $(ADB)
	$(eval FIRST_RUN := $(shell $(ADB) shell pm list packages | grep -c com.oriondev.moneywallet.dev))
	echo $(FIRST_RUN)

	@if [ "$(FIRST_RUN)" == "2" ]; then \
		echo "Resetting the data of the apk"; \
		$(ADB) shell pm clear com.oriondev.moneywallet.dev; \
	else \
	...
	$(ADB) shell am instrument -w -e class $(TEST_NAME) com.oriondev.moneywallet.dev.test/androidx.test.runner.AndroidJUnitRunner | tee $(@)
...
copy-shadows :
	cp -v ./shadows/*.java app/src/carvedTest/com/oriondev/moneywallet
